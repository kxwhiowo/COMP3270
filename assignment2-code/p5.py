...
def choose_better_direction_for_ghost(map, available_directions, position, player, depth):
    if len(available_directions) == 0:
        return None
    return minimax(available_directions, map, position, depth, player)
...
def evaluate_func(map):
    ...
    closest_ = calculate_closest_score(player_position, positions_ghost,food_position)
    return closest_
...
def check_nearest_food_position(map, player_position):
    ...
    if len(sorted_points) == 0:
        return 0
    return math.dist(player_position, sorted_points[0])
